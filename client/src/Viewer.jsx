...
export default function Viewer({ streamId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
            ...
            if (user && response.data.broadcasterId && profileData) {
              setIsFollowing(
                profileData.following.some(
                  (f) => f._id === response.data.broadcasterId._id
                )
              );
            }
          } catch (err) {
        ...
        // ✅ Viewer only handles offers, sends back answers
        ...
        // ✅ Remove local offer creation (no extra negotiation)
  ...
  // ✅ Don’t add comment locally — just emit and wait for server broadcast
      ...
      <div style={{ position: "relative", width: "70vw", height: "100vh" }}>
        ...
        >
          {!connected ? (
            ...
            <small style={{ color: "red", fontWeight: "bold" }}>● Live</small>
          )}
              ...
              <div key={i} style={{ marginBottom: "8px" }}>
                ...
                {user && c.userId !== user._id && (
                  <button
                    ...
                    style={{
                      marginLeft: "10px",
                      backgroundColor: followingList.includes(c.userId)
                        ? "#007bff"
                        : "#28a745",
                      color: "#fff",
                      border: "none",
